...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'audit', label: 'Audit Trail', icon: Activity, roles: ['ADMIN', 'MANAGER', 'AUDITOR'] },
...
export function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile menu toggle button - shows when sidebar is closed on mobile */}
      <button
        className="lg:hidden fixed top-6 left-6 z-50 flex items-center gap-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 cursor-pointer transition-colors bg-white border border-gray-200 rounded-lg px-2 py-1.5 shadow-sm"
        onClick={() => onClose?.()}
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-200" style={{ minHeight: '72px' }}>
          <div className="w-10 h-10 rounded-lg bg-primary-blue flex items-center justify-center flex-shrink-0">
            <ShieldCheck className="h-5 w-5 text-white" />
          </div>
          {!isCollapsed && (
            <div className="side-brand-text min-w-0">
              <div className="font-heading font-bold text-gray-900 truncate">SecureChain</div>
              <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Control Center</div>
            </div>
          )}
        </div>
            ...
            return (
              <NavLink
                ...
                className={({ isActive: active }) => cn(
                  ...
                  active ? 'bg-primary-blue/10 text-primary-blue font-semibold' : 'text-gray-700',
          ...
          <div className="flex items-center gap-3 px-1 py-2 mb-3">
            ...
            {!isCollapsed && (
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-gray-500 uppercase tracking-wider">{displayRole(user?.role)}</div>
              </div>
            )}
          </div>
          <Button
            ...
            onClick={() => logout()}
            leftIcon={!isCollapsed ? <LogOut className="h-4 w-4" /> : undefined}
          >
            {!isCollapsed && 'Logout'}
